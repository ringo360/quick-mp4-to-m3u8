...
async function main() {
	...
	//todo: 自動でディレクトリ作成
	const run_cmd = `ffmpeg -i target/${file} -c:v copy -c:a copy -f hls -hls_time 9 -hls_playlist_type vod -hls_segment_filename "out/output-${file_x}-%5d.ts" out/output-${file_x}.m3u8`;
